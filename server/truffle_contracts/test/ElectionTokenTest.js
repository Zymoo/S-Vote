...
contract('ElectionToken', function(accounts) {
  ...
  it('initializes the contract with the correct values', function() {
    return ElectionToken.deployed().then(function(instance) {
      ...
      assert.equal(name, 'Election Token', 'has the correct name');
      return tokenInstance.symbol();
    }).then(function(symbol) {
      assert.equal(symbol, 'SET', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard) {
      assert.equal(standard, 'Election Token v1.0', 'has the correct standard');
    });
  })
